...
class ScrollAnimations {
    ...
    prepareElements() {
        ...
        if (heroSection) {
            heroSection.classList.remove('animate-on-scroll', 'fade-up', 'scale-up', 'slide-left', 'slide-right');
            heroSection.querySelectorAll('*').forEach(el => {
                el.classList.remove('animate-on-scroll', 'fade-up', 'scale-up', 'slide-left', 'slide-right', 'stagger');
            });
        }
        ...
        if (aboutSection) {
            aboutSection.querySelectorAll('.grid.grid-cols-1.md\\:grid-cols-3 > div').forEach((cred, i) => {
                cred.classList.add('animate-on-scroll', 'fade-up', 'stagger');
                cred.style.setProperty('--stagger-index', i);
            });
            
            // Science cards
            aboutSection.querySelectorAll('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-3 > div').forEach((card, i) => {
                card.classList.add('animate-on-scroll', 'scale-up', 'stagger');
                card.style.setProperty('--stagger-index', i);
            });
        }
        ...
        document.querySelectorAll('img[src*="cgm"], img[src*="chart"], img[alt*="CGM"]').forEach(img => {
        ...
        const mainObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (entry.target.classList.contains('animate-number') && !entry.target.classList.contains('counted')) {
