...
1. **Fresh session only.** Check context with `mcp__ccd_session_mgmt__get_usage`. Over ~100k, or this session already worked another task → stop and say to run `/quick <ID>` in a new session. Also needs to run from a worktree slot exactly like `/work` — step 5 below delegates to `/work` step 3, which now refuses from the main checkout.
