...
1. Find the task with `Glob .claude/tasks/*/<ID>/task.md` (its folder name above the id is the category). A sub-task id (`SN-012.2`) → refuse: sub-tasks close with their parent; mark a single sub-task `status: done` with `/orchestrate force` only if the user insists.
