...
describe('🚨 CRITICAL BUSINESS SCENARIOS', () => {
  ...
  describe('1️⃣ КРИТИЧНО: Клиент может посмотреть доступные услуги', () => {
    test('Services таблица существует и содержит услуги', async () => {
      ...
      expect(
        service.serviceName_LV || service.serviceName_RU || service.serviceName_EN
      ).toBeDefined();
  ...
  describe('3️⃣ КРИТИЧНО: Валидация данных бронирования', () => {
    ...
    test('Отклоняет дату в прошлом', () => {
      ...
      const errors = validateBooking(booking);
      expect(
        errors.some((e) => e.toLowerCase().includes('past') || e.toLowerCase().includes('date'))
      ).toBe(true);
    });
  ...
  describe('4️⃣ КРИТИЧНО: Система предотвращает двойное бронирование', () => {
    test('Проверка занятости слота перед бронированием', () => {
      const existingBookings = [{ date: FUTURE_DATE, time: '14:00', status: 'confirmed' }];
    ...
    test('Разрешает бронирование на место отмененного', () => {
      const existingBookings = [{ date: FUTURE_DATE, time: '14:00', status: 'cancelled' }];
...
function generateTimeSlots(workingHours, duration, existingBookings) {
  ...
  for (let minutes = startMinutes; minutes < endMinutes; minutes += step) {
    ...
    const isBooked = existingBookings.some((b) => b.time === timeSlot && b.status !== 'cancelled');
